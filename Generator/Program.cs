...
FormulaSet tsuv = new FormulaSet("s=0.5*(v-u)*t", "t=2*s/(v-u)", "v=u+2*s/t", "u=v-2*s/t", startSpeed, endSpeed, distance, time);
FormulaSet tuva = new FormulaSet("a=(v-u)/t", "v=u+a*t", "u=v-a*t", "t=(v-u)/a", startSpeed, endSpeed, acceleration, time);
